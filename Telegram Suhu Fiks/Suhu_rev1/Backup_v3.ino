...
void loop() {
  ...
  float kelembapan = dht.readHumidity(); // pembacaan kelembapan
  ...
  if (line.indexOf("KELEMBAPAN") > 0) {
    String kelembapan = "/bot901305640:AAFs34qeuDPSKezjS6gr62yV3_brTtqIFj0/sendMessage?chat_id=875288625&text=Kelembapan%20Sekarang%20:%20";
  ...
  if (line.indexOf("ON_A") > 0) {   // on kipas
    digitalWrite(16, HIGH);
  }
  if (line.indexOf("ON_B") > 0) {   //on blower
    digitalWrite(4, HIGH);
  }
  if (line.indexOf("OFF_A") > 0) {  //off kipas
    digitalWrite(16, LOW);
  }
